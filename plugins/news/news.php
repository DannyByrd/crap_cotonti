...
{
	$v = explode('|', trim($v));
	$code =  (substr($v[0], -1) == 1 || substr($v[0], -1) == 2 ||substr($v[0], -1) == 3) ? substr($v[0], 0, -1) : $v[0];
	if (isset($structure['page'][$code]))	
	{
		$c = (empty($c)) ? $v[0] : $c;
...
if (count($cats) > 0)
{
	...
	foreach ($cats as $k => $v)
	{	
		$cat_code =  (substr($v[0], -1) == 1 || substr($v[0], -1) == 2 ||substr($v[0], -1) == 3) ? substr($v[0], 0, -1) : $v[0];
		$cat = ($catn == 0) ? $c : $cat_code;
