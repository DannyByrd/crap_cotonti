...
$(document).ready(function(){
		$('#catgenerator select[name=newsmaincat] option').each(function(){
			var $option = $(this);
			var val = $option.val();
			var text = $option.text();
			
			$option.after('<option value="' + val + '3">' + text + '(copy 3)</option>');
			$option.after('<option value="' + val + '2">' + text + '(copy 2)</option>');
			$option.after('<option value="' + val + '1">' + text + '(copy 1)</option>');
		});

	$('[name=category]').closest('tr').hide(); //closest  Получает набор, содержащий ближайшие родительские элементы, которые удовлетворяют указанному селектору, включая начальный элемент.
