...
class mavatar
{
	...
	public function generate_tags($mavatar)
	{
		...
		{
			if($key_p == 'desc'){
				if(((int)strpos($val_p, '|')) > 0){
					$tmp= explode('|', $val_p);
					foreach ($tmp as $key_tmp => $val_tmp)
					{
						$keyx = mb_strtoupper('desc'.($key_tmp+1));
						$curr_mavatar[$keyx] = $val_tmp;
					}
				} 
			}
...
function cot_mav_thumb($object, $width, $height, $resize = 'crop', $filter = '', $quality = 85)
{
	...
	if (file_exists($thumb_file))
	{
		...
		$noimg_file = $object['thumbpath'].$object['filename'].'_'.$width.'_'.$height.'_'.$resize;
		$noimg_file .= (!empty($filter)) ? '_'.$filter : '';
		$noimg_file .= '.'.$object['fileext'];
		if (file_exists($noimg_file))
		{
			return $noimg_file;
		} else {
			$thumb_file = $noimg_file;
		}
	}
	...
	if($cfg['plugin']['mavatars']['usewatersign'] === '1'){
		
		$watter_path = 'datas/mavatars/watermark.png';
		
		if(file_exists($watter_path) && file_exists($thumb_file))
		{

			
			switch ($object['fileext']) {
				case 'gif':
					$im2=imageCreateFromGIF($thumb_file);
					break;
				case 'png':
					$im2=imageCreateFromPNG($thumb_file);
					break;
				default:
					$im2=imageCreateFromJPEG($thumb_file);
					break;
			}
			
			
			$pos_underscore = strpos($thumb_file, '_'); 
			$output_img = substr_replace($thumb_file, '_wattermark_', $pos_underscore,0);
			...
			$im1 = changesize("datas/mavatars/watermark.png",$size_x,$size_y,$resize);

			
			imageCopy($im2,$im1,0,0,0,0,$size_x,$size_y);

			
			imagePNG($im2, $output_img);
...
function changesize($filename,$width,$height,$resize){

	/**
	 *@var resize - doesn't work!
	*/
	...
	*/
	...
	// вывод 
	 imagepng($image_p, 'datas/mavatars/img/output.png');
	 return $image_p;
