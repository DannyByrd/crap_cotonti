...
global $temp,$db_x;
	...
	$user_money = cot_import('um_usermoney','P','NUM');
	...
	if (mb_strlen($user_money) < 1) 
	    cot_error('aut_usermoneytooshort', 'um_usermoney');
...
if (!cot_error_found())
	{
		...
		if(!is_null($user_money)){

		 $now = time();
		
		 $query = $db->prepare("INSERT INTO $db_x"."payments (pay_userid, pay_status, pay_cdate, pay_pdate, pay_adate, pay_summ, pay_desc, pay_area, pay_code, pay_time) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
		 $query->execute(array($userid,'done',$now,$now,$now,$user_money,'Пополнение счета','balance','admin',0));
		...
		{
			cot_message(um_build_string($L['um_user'], $umuser['user_name'], $L['um_successcreation'],true));
