...
function cot_apply_rwr()
{
	...
	if (isset($_GET['rwr']) && !empty($_GET['rwr']))
	{
		...
		$defaultmodule = (array_key_exists($preset_name, $structure)) ? $preset_name : 'page';
		$pluginsPageWithHtml = array(
			'contact',
			'search',
		);

		$modulePageWithSlash = array(
			'afisha',
			'blogs',
			'board',
			'firms',
			'page', // is the default module
			'resume',
			'products',
			'vacancies',
		...
		if ($count == 1)
		{
			if (isset($structure[$defaultmodule][$path[0]]) || $path[0] == 'unvalidated' || $path[0] == 'saved_drafts')
			{
				// Is a category
				if(mb_substr($_GET['rwr'], -1) == '/'){
					$_GET['e'] = $defaultmodule;
					$_GET['c'] = $path[0];	
				} else {
					cot_set404();
				}
			}
			elseif (file_exists($cfg['modules_dir'] . '/' . $path[0]) || file_exists($cfg['plugins_dir'] . '/' . $path[0])  || file_exists($cfg['plugins_dir'] . '/' . substr($path[0], 0, -5)))
			{
				if(file_exists($cfg['plugins_dir'] . '/' . $path[0]) || file_exists($cfg['plugins_dir'] . '/' . substr($path[0], 0, -5))){
					//Is an plugin
					if(in_array(substr($path[0], 0, -5), $pluginsPageWithHtml)){
						//Is an plugin page with .html
						$_GET['e'] = substr($path[0], 0, -5);
					} else {
						$_GET['e'] = $path[0];
					}
					...
					// is a module
					if(in_array($path[0], $modulePageWithSlash) && mb_substr($_GET['rwr'], -1) == '/'){
						$_GET['e'] = $path[0];
					} else {
						if(!in_array($path[0], $modulePageWithSlash)){
							$_GET['e'] = $path[0];
						} else {
							cot_set404();
						}
			...
			elseif (in_array($path[0], array('register', 'passrecover')))
			{
		...
		else
		{
			...
			$last = $count - 1;
			$ext = (isset($structure[$defaultmodule][$path[0]])) ? $defaultmodule : $path[0];
			$_GET['e'] = $ext;
			if($path[$last-1] == 'pag'){
				$_GET['d'] = $path[$last];
				$_GET['c'] = $path[$last - 2];
			} 
			else if (isset($structure[$ext][$path[$last]]))
			{
				// Is a category
				if(in_array($ext, $modulePageWithSlash)){
					if(substr($_GET['rwr'], -1) == '/'){
						$_GET['c'] = $path[$last];
					} else {
						cot_set404();
					}
			...
			{
				// Is a page/item
				if ($ext == $defaultmodule || $count > 2)
				{
					$_GET['c'] = $path[$last - 1];
				}

				if(in_array($ext, $modulePageWithSlash)){
					$path_last =  substr($path[$last], 0, -5);
				} else {
					$path_last = $path[$last];
				}

				if (is_numeric($path_last))
				{
...
function cot_url_custom($name, $params = '', $tail = '', $htmlspecialchars_bypass = false)
{
	...
	if($cfg['plugin']['urleditor']['preset'] == 'items-depth2' && $name == 'products' && isset($params['c'])){
		unset($params['c']);
	}
