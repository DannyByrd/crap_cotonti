...
{
	global $cfg, $structure, $url_rules_for_extansions, $curr_module;
	...
	if (isset($_GET['rwr']) && !empty($_GET['rwr'])/* && preg_match('`^[\w\p{L}/\-_\ \+\.]+?$`u', $_GET['rwr'])*/)
	{
		...
		if(in_array($curr_module, $url_rules_for_extansions)){
			if($preset_name == 'palmira' && substr($path[$count-1], -1) == '/'){
				$path = array('404');
			}

			// open urls only with .html
			if(substr($preset_name, -4) == 'html'){
				$path[$count-1] = substr($path[$count-1], 0, -5);
			}

			if(!$path[$count-1]) $path = array('404');
		} else {
			// open with .html and without .html
			if(substr($path[$count-1], -5) == '.html'){
				$path[$count-1] = substr($path[$count-1], 0, -5);
			}
		}
		...
		else
		{
			...
			if (isset($structure[$ext][$path[$last]]))
			{
				// Is a category
				$_GET['c'] = $path[$last];
			}elseif(strpos($path[$last], 'pag-') === 0){
				$_GET['d'] = substr($path[$last], 4);
				$_GET['c'] = $path[$last - 1];
			}
			else
			{
				...
				if ($ext == $defaultmodule || $count > 2)
				{
					...
					$allowed_exts=array('products');
					if($defaultmodule != 'products' && in_array($ext, $allowed_exts) && $cfg['plugin']['urleditor']['preset'] == 'items-depth2'){
						if($count <= 2){
