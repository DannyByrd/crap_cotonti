...
function autoalias2_update($title, $id)
{
	...
	do
	{
		...
		if (!$cfg['plugin']['autoalias2']['prepend_id']
			&& $db->query("SELECT COUNT(*) FROM $db_products
				WHERE prd_alias = '$alias' AND prd_id != $id AND prd_cat = '" . $c . "'")->fetchColumn() > 0)
		{
			$duplicate = true;
		}
		else
		{
			$db->update($db_products, array('prd_alias' => $alias), "prd_id = $id");
			$duplicate = false;
		}
	}
