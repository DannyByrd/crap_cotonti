...
function autoalias2_update($title, $id)
{
	...
	do
	{
		...
		if (!$cfg['plugin']['autoalias2']['prepend_id']
			&& $db->query("SELECT COUNT(*) FROM $db_rezume
				WHERE rez_alias = '$alias' AND rez_id != $id AND rez_cat = '$c'")->fetchColumn() > 0)
		{
			$duplicate = true;
		}
		else
		{
			$db->update($db_rezume, array('rez_alias' => $alias), "rez_id = $id");
			$duplicate = false;
		}
	}
