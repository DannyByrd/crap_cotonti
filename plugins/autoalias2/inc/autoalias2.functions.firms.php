...
function autoalias2_update($title, $id)
{
	...
	do
	{
		...
		if (!$cfg['plugin']['autoalias2']['prepend_id']
			&& $db->query("SELECT COUNT(*) FROM $db_firms
				WHERE firm_alias = '$alias' AND firm_id != $id AND firm_cat = '$c'")->fetchColumn() > 0)
		{
			$duplicate = true;
		}
		else
		{
			$db->update($db_firms, array('firm_alias' => $alias), "firm_id = $id");
			$duplicate = false;
		}
	}
