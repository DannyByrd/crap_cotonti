...
function autoalias2_update($title, $id)
{
	...
	do
	{
		...
		if (!$cfg['plugin']['autoalias2']['prepend_id']
			&& $db->query("SELECT COUNT(*) FROM $db_afisha
				WHERE event_alias = '$alias' AND event_id != $id AND event_cat = '$c'")->fetchColumn() > 0)
		{
			$duplicate = true;
		}
		else
		{
			$db->update($db_afisha, array('event_alias' => $alias), "event_id = $id");
			$duplicate = false;
		}
	}
