...
if(is_null($c)) $c = $GLOBALS["_GET"]["al"];
//var_dump($c);
...
/* === Hook === */
foreach (cot_getextplugins('map.list.first') as $pl)
{
	include $pl;
}
/* ===== */
...
$catpatharray = cot_map_buildpath($c);
...
/* === Hook === */
foreach (cot_getextplugins('map.list.query') as $pl)
{
	include $pl;
}
/* ===== */
...
if ((!$cfg['easypagenav'] && $durl > 0 && $cfg['firms']['maxrowsperpage'] > 0 && $durl % $cfg['firms']['maxrowsperpage'] > 0)
	|| ($d > 0 && $d >= $totallines))
{
	cot_redirect(cot_url('firms', $list_url_path + array('dc' => $dcurl)));
}

$pagenav = cot_pagenav('firms', $list_url_path + array('dc' => $dcurl), $d, $totallines, $cfg['firms']['maxrowsperpage']);

if(empty($c)){
	$cfg_seotitle = trim($cfg['firms']['seo_title']);
...
$mskin = cot_tplfile(array('map', 'list', $cat['tpl']), 'plug');

/* === Hook === */
foreach (cot_getextplugins('map.list.main') as $pl)
{
	include $pl;
}
/* ===== */
...
$t->assign(array(
	...
	'LIST_CATURL' => cot_url('map', $list_url_path),
...
foreach ($cot_extrafields[$db_firms] + array('title' => 'title', 'key' => 'key', 'date' => 'date', 'owner' => 'owner', 'count' => 'count') as $row_k => $row_p)
{
	...
	$url_asc = cot_url('map',  array('s' => $row_k, 'w' => 'asc') + $list_url_path);
	$url_desc = cot_url('map', array('s' => $row_k, 'w' => 'desc') + $list_url_path);
...
$jj = 0;
/* === Hook - Part1 : Set === */
$extp = cot_getextplugins('map.list.loop');
/* ===== */
$sqllist_rowset = $sqllist->fetchAll();

$sqllist_rowset_other = false;
/* === Hook === */
foreach (cot_getextplugins('map.list.before_loop') as $pl)
{
	include $pl;
}
/* ===== */
...
if(!$sqllist_rowset_other)
{
	...
	{
		$jj++;
		$t->assign(cot_generate_maptags($firm, 'LIST_ROW_', $cfg['firms']['truncatetext'], $usr['isadmin']));
		$t->assign(array(
...
$t->assign(array(
	'LIST_FIRMMARKS' => cot_map_getmarks ('firms', $c),
));


/* === Hook === */
foreach (cot_getextplugins('map.list.tags') as $pl)
{
	include $pl;
}
/* ===== */
