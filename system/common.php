...
$url_pars = explode('/', (mb_strrpos($_GET['rwr'], '/') === mb_strlen($_GET['rwr']) - 1) ? mb_substr($_GET['rwr'], 0, -1) : $_GET['rwr']);
$curr_module = substr($url_pars[0], -5) == '.html' ? substr($url_pars[0], 0, -5) : $url_pars[0];
if(isset($structure['page'][$curr_module])){
	$curr_module = 'page';
}

if(isset($structure['page'])){
	$url_rules_for_extansions = array_merge(array_keys($structure), array_keys($structure['page']), array('contact', 'search'));
} else {
	$url_rules_for_extansions = array_merge(array_keys($structure), array('contact', 'search'));
}
