...
else
{
	...
	{
		$field_cats = cot_import('field_cats', 'P', 'ARR');
		...
		$field['field_cats'] = isset($field_cats) ? serialize($field_cats) : '';
		...
		{
			if (cot_extrafield_add($n, $field['field_name'], $field['field_type'], $field['field_html'], $field['field_variants'], $field['field_default'], $field['field_required'], $field['field_parse'], $field['field_description'], $field['field_params'], $field['field_enabled'], $field['field_noalter'], NULL, $field['field_cats']))
			{
	...
	elseif ($a == 'upd' && !empty($_POST))
	{
		...
		if (is_array($field_name))
		{
			foreach ($field_name as $k => $v)
			{
				...
				$field['field_cats'] = isset($field_cats[$k]) ? serialize($field_cats[$k]) : '';
				...
				if ($field != $cot_extrafields[$n][$field['field_name']] && !empty($field['field_name']) && !empty($field['field_type']))
				{
					...
					$fieldresult = cot_extrafield_update($n, $k, $field['field_name'], $field['field_type'], $field['field_html'], $field['field_variants'], $field['field_default'], $field['field_required'], $field['field_parse'], $field['field_description'], $field['field_params'], $field['field_enabled'], NULL, $field['field_cats']);
					if ($fieldresult == 1)
					{
	...
	$field_types = array('input', 'inputint', 'currency', 'double', 'textarea', 'select', 'checkbox', 'radio', 'datetime', 'country', 'range', 'checklistbox', 'file', 'filter'/* , 'filesize' */);

	$modules = array_keys($structure);
	$selectbox_cats = array('all' => 'Все категории');

	$module_code = $extra_whitelist[$n]['code'];
	if($structure[$module_code]){
		foreach ($structure[$module_code] as $item) {
			if(!in_array($item['path'], $modules)){
				$selectbox_cats[$item['path']] = $item['tpath'];
			}
		}
	}
	...
	foreach ($res->fetchAll() as $row)
	{
		...
		$t->assign(array(
			...
			'ADMIN_EXTRAFIELDS_SELECT_CATS' => cot_selectbox(unserialize($row['field_cats']), 'field_cats['.$row['field_name'].'][]', array_keys($selectbox_cats), array_values($selectbox_cats), false, 'class="field_cats" disabled="disabled" multiple size="10" style="display:none;"'),
	...
	$t->assign(array(
		...
		'ADMIN_EXTRAFIELDS_SELECT_CATS' => cot_selectbox('', 'field_cats[]', array_keys($selectbox_cats), array_values($selectbox_cats), false, 'class="field_cats" disabled="disabled" multiple size="10" style="display:none;"'),
