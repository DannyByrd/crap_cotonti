...
else
{
	...
	if ($a == 'update' && !empty($_POST))
	{
		...
		$rstructureseotitle = cot_import('rstructureseotitle', 'P', 'ARR');
		$rstructuremetadesc = cot_import('rstructuremetadesc', 'P', 'ARR');
		$rstructuredesc = cot_import('rstructuredesc', 'P', 'ARR');
		$rstructuretext = cot_import('rstructuretext', 'P', 'ARR');
		...
		foreach ($rstructurecode as $i => $k)
		{
			...
			$rstructure['structure_seotitle'] = cot_import($rstructureseotitle[$i], 'D', 'TXT');
			$rstructure['structure_metadesc'] = cot_import($rstructuremetadesc[$i], 'D', 'HTM');
			$rstructure['structure_desc'] = cot_import($rstructuredesc[$i], 'D', 'HTM');
			$rstructure['structure_text'] = cot_import($rstructuretext[$i], 'D', 'HTM');
	...
	elseif ($a == 'add' && !empty($_POST))
	{
		...
		$rstructure['structure_seotitle'] = cot_import('rstructureseotitle', 'P', 'TXT');
		$rstructure['structure_metadesc'] = cot_import('rstructuremetadesc', 'P', 'TXT');
		$rstructure['structure_desc'] = cot_import('rstructuredesc', 'P', 'HTM');
		$rstructure['structure_text'] = cot_import('rstructuretext', 'P', 'TXT');
		...
		// $rstructure['structure_seotitle'] || cot_error('adm_structure_seotitle_required', 'rstructureseotitle');
	...
	foreach ($sql->fetchAll() as $row)
	{
		...
		$t->assign(array(
			...
			'ADMIN_STRUCTURE_SEOTITLE' => cot_inputbox('text', 'rstructureseotitle['.$structure_id.']', $row['structure_seotitle'], 'size="32" maxlength="255"'),
			'ADMIN_STRUCTURE_METADESC' => cot_textarea('rstructuremetadesc['.$structure_id.']', $row['structure_metadesc'], 10, 160, ''),
			'ADMIN_STRUCTURE_DESC' => cot_textarea('rstructuredesc['.$structure_id.']', $row['structure_desc'], 10, 160, '', 'input_textarea_editor'),
			'ADMIN_STRUCTURE_TEXT' => cot_textarea('rstructuretext['.$structure_id.']', $row['structure_text'], 10, 160, '', 'input_textarea_editor'),
	...
	if (!$id && empty($al))
	{
		...
		$t->assign(array(
			...
			'ADMIN_STRUCTURE_SEOTITLE' => cot_inputbox('text', 'rstructureseotitle', null, 'size="64" maxlength="100"'),
			'ADMIN_STRUCTURE_METADESC' => cot_textarea('rstructuremetadesc', null, 10, 160, ''),
			'ADMIN_STRUCTURE_DESC' => cot_textarea('rstructuredesc', null, 10, 160, '', 'input_textarea_editor'),
			'ADMIN_STRUCTURE_TEXT' => cot_textarea('rstructuretext', null, 10, 160, '', 'input_textarea_editor'),
