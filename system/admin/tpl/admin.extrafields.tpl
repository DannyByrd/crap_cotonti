...
//<![CDATA[
	...
	$(document).ready(function(){
		$('body').on("change", '.exfldtype', function(){
			...
			if(exvalid == 'select' || exvalid == 'radio' || exvalid == 'checklistbox' || exvalid == 'file' || exvalid == 'filter')
			{
				if (exvalid == 'file') {
			...
			switch(exvalid)
			{
				...
				case 'filter':
					$(exParent).find('.exfldparams').attr('title',exseparator);
					$(exParent).find('.exfldparams').removeAttr("disabled");
					$(exParent).find('.field_cats').removeAttr("disabled");
					$(exParent).find('.field_cats, .field_cats_label').show();
					break;
			}
		...
		<table class="cells">
			...
			<tr id="ex{ADMIN_EXTRAFIELDS_ROW_ID}">
				...
				<td class="{ADMIN_EXTRAFIELDS_ROW_ODDEVEN}">
					...
					<p class="small field_cats_label" style="display: none;">{PHP.L.adm_extrafield_select_cats}</p>
						{ADMIN_EXTRAFIELDS_SELECT_CATS}
				</td>
