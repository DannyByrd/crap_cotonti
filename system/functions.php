 ...
 * @return bool
 */
function cot_mail($fmail, $subject, $body, $headers='', $customtemplate = false, $additional_parameters = null, $html = false, $attachments = array())
{
	...
	{
		return cot_mail_custom($fmail, $subject, $body, $headers, $customtemplate, $additional_parameters, $html, $attachments);
	}
...
function cot_mail_custom($fmail, $subject, $body, $headers='', $customtemplate = false, $additional_parameters = null, $html = false, $attachments = array())
{
    global $cfg;
    ...
    else
    {
        ...
        $sitemaintitle = mb_encode_mimeheader($cfg['maintitle'], 'UTF-8', 'B', "\n");
        $headers .= (empty($headers)) ? "From: \"" . $sitemaintitle . "\" <" . $cfg['adminemail'] . ">" . $newline . "Reply-To: <" . $cfg['adminemail'] . ">" . $newline : $headers;
        $type_body = $html ? "html" : "plain";
        $headers .= 'X-Mailer: PHP/' . phpversion() . $newline;
        $headers .= 'Content-Type: multipart/related; boundary="' . $boundary . '"' . $newline . $newline;
        ...
        if (!$customtemplate)
        {
            ...
            $subject = cot_title($cfg['subject_mail'], $subject_params, false);
            $text = cot_title(str_replace("\r\n", "\n", $cfg['body_mail']), $body_params, false);
        }

        if (!$html) {
            $body  = '--' . $boundary . $newline;
            $body .= 'Content-Type: text/plain; charset="utf-8"' . $newline;
            $body .= 'Content-Transfer-Encoding: 8bit' . $newline . $newline;
            $body .= $text . $newline;
        } else {
            $body  = '--' . $boundary . $newline;
            $body .= 'Content-Type: multipart/alternative; boundary="' . $boundary . '_alt"' . $newline . $newline;
            $body .= '--' . $boundary . '_alt' . $newline;
            $body .= 'Content-Type: text/plain; charset="utf-8"' . $newline;
            $body .= 'Content-Transfer-Encoding: 8bit' . $newline . $newline;
            ...
            if ($text) {
                ...
                $body .= 'This is a HTML email and your email client software does not support HTML email!' . $newline;
            }
        ...
        if(count($attachments)){
            foreach ($attachments as $attachment) {
                if (file_exists($attachment['tmp_name'])) {
                    ...
                    $content = fread($handle, filesize($attachment['tmp_name']));
                    fclose($handle);

                    $body .= '--' . $boundary . $newline;
                    $body .= 'Content-Type: application/octet-stream; name="' . basename($attachment['name']) . '"' . $newline;
                    $body .= 'Content-Transfer-Encoding: base64' . $newline;
                    $body .= 'Content-Disposition: attachment; filename="' . basename($attachment['name']) . '"' . $newline;
                    $body .= 'Content-ID: <' . basename(urlencode($attachment['name'])) . '>' . $newline;
                    $body .= 'X-Attachment-Id: ' . basename(urlencode($attachment['name'])) . $newline . $newline;
...
function cot_load_structure()
{
	...
	foreach ($sql->fetchAll() as $row)
	{
		...
		$structure[$row['structure_area']][$row['structure_code']] = array(
			...
			'seotitle' => $row['structure_seotitle'],
			'desc' => $row['structure_desc'],
			'text' => $row['structure_text'],
			'metadesc' => $row['structure_metadesc'],
