...
if (!COT_AJAX)
{
	...
	if ($cfg['enablecustomhf'])
	{
		...
		if($_SERVER["REQUEST_URI"] == '/') $ext = 'index';
		$c = isset($c) ? $c : '';
		$al = isset($al) ? $al : '';
		$m = isset($m) && ($m == 'add' || $m == 'edit') ? $m : '';

		if($m){
			$mtpl_base = (defined('COT_PLUG') && !empty($ext)) ? array('header', $ext, $m) : array('header', $ext, $m);
		}else{
			$mtpl_base = (defined('COT_PLUG') && !empty($ext)) ? array('header', $ext, $c, $al) : array('header', $ext, $c, $al);
		}
	}
	else
	{
	...
	$t->assign(array(
		...
		'HEADER_HREFLANG1' =>$out['alt_hreflang1'],
		'HEADER_ALTURL1' =>$out['alt_url1'],
