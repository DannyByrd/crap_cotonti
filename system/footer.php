...
if (!COT_AJAX)
{
	...
	if ($cfg['enablecustomhf'])
	{
		...
		if($m){
			$mtpl_base = (defined('COT_PLUG') && !empty($ext)) ? array('footer', $ext, $m) : array('footer', $ext, $m);
		}else{
			$mtpl_base = (defined('COT_PLUG') && !empty($ext)) ? array('footer', $ext, $c, $al) : array('footer', $ext, $c, $al);
		}
