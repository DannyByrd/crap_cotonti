...
/* === Hook === */
foreach (cot_getextplugins('rezume.list.first') as $pl)
{
	include $pl;
}
/* ===== */
...
$cfg['rezume']['truncateREZtext'] = (empty($c) || $c == 'unvalidated' || $c == 'saved_drafts') ?
	$cfg['rezume']['cat___default']['truncateREZtext'] :
	$cfg['rezume']['cat_' . $c]['truncateREZtext'];
...
{
	$cat['title'] = $L['Rezume'];
	$cat['desc'] = $cfg['rezume']['description'];
	$s = 'date';
	$w = 'desc';
}
...
{
	$where['date'] = "(rez_expire = 0 OR rez_expire > {$sys['now']})";
}
...
$list_url = cot_url('rezume', $list_url_path);
...
$catpatharray = cot_rezume_buildpath($c);
...
/* === Hook === */
foreach (cot_getextplugins('rezume.list.query') as $pl)
{
	include $pl;
}
/* ===== */
...
if(empty($sql_rez_string))
{
	...
	$sql_rez_count = "SELECT COUNT(*) FROM $db_rezume as p $join_condition $where";
	$sql_rez_string = "SELECT p.*, u.* $join_columns
...
if ((!$cfg['easypagenav'] && $durl > 0 && $cfg['rezume']['maxrowsperpage'] > 0 && $durl % $cfg['rezume']['maxrowsperpage'] > 0)
	|| ($d > 0 && $d >= $totallines))
{
	cot_redirect(cot_url('rezume', $list_url_path + array('dc' => $dcurl)));
}

$pagenav = cot_pagenav('rezume', $list_url_path + array('dc' => $dcurl), $d, $totallines, $cfg['rezume']['maxrowsperpage']);

if(empty($c)){
	$cfg_seotitle = trim($cfg['rezume']['seo_title']);
...
$out['keywords'] = (!empty($cfg['rezume']['cat_' . $c]['keywords'])) ? $cfg['rezume']['cat_' . $c]['keywords'] : $cfg['rezume']['cat___default']['keywords'];

// Building the canonical URL
$out['canonical_uri'] = cot_url('rezume', $pageurl_params);

$_SESSION['cat'] = $c;

$mskin = cot_tplfile(array('rezume', 'list', $cat['tpl']));

/* === Hook === */
foreach (cot_getextplugins('rezume.list.main') as $pl)
{
	include $pl;
}
/* ===== */
...
$t->assign(array(
	...
	'LIST_CAT' => $c,
	'LIST_CAT_RSS' => cot_url('rss', "m=rezume&c=$c"),
	'LIST_CATTITLE' => $cat['title'],
	'LIST_CATPATH' => $catpath,
	'LIST_CATSHORTPATH' => $catpath_short,
	'LIST_CATURL' => cot_url('rezume', $list_url_path),
...
{
	$t->assign(array(
		'LIST_SUBMITNEWFIRM' => cot_rc('rez_submitnewrezume', array('sub_url' => cot_url('rezume', 'm=add&c='.$c))),
		'LIST_SUBMITNEWREZ_URL' => cot_url('rezume', 'm=add&c='.$c)
	));
}
...
$pagenav_cat = cot_pagenav('rezume', $list_url_path + array('d' => $durl), $dc, count($allsub), $cfg['rezume']['maxlistsperpage'], 'dc');
...
$jj = 0;
/* === Hook - Part1 : Set === */
$extp = cot_getextplugins('rezume.list.loop');
/* ===== */
$sqllist_rowset = $sqllist->fetchAll();

$sqllist_rowset_other = false;
/* === Hook === */
foreach (cot_getextplugins('rezume.list.before_loop') as $pl)
{
	include $pl;
}
/* ===== */
...
/* === Hook === */
foreach (cot_getextplugins('rezume.list.tags') as $pl)
{
	include $pl;
}
/* ===== */
