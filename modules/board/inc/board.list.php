...
/* === Hook === */
foreach (cot_getextplugins('board.list.first') as $pl)
{
	include $pl;
}
/* ===== */
...
$cfg['board']['truncateadvtext'] = (empty($c) || $c == 'unvalidated' || $c == 'saved_drafts') ?
	$cfg['board']['cat___default']['truncateadvtext'] :
	$cfg['board']['cat_' . $c]['truncateadvtext'];

$where = array();
$params = array();

$where['state'] = "adv_state=0";
$where['expire'] = "(adv_expire=0 OR adv_expire > {$sys['now']})";

if(empty($c))
{
	$cat['title'] = $L['Board'];
	$cat['desc'] = $cfg['board']['description'];
}
...
{
	$where['date'] = "(adv_expire = 0 OR adv_expire > {$sys['now']})";
}
...
$list_url = cot_url('board', $list_url_path);
...
$catpatharray = cot_board_buildpath($c);
...
/* === Hook === */
foreach (cot_getextplugins('board.list.query') as $pl)
{
	include $pl;
}
/* ===== */
...
if(empty($sql_adv_string))
{
	...
	$sql_adv_count = "SELECT COUNT(*) FROM $db_board as p $join_condition $where";
	$sql_adv_string = "SELECT p.*, u.* $join_columns
...
if ((!$cfg['easypagenav'] && $durl > 0 && $cfg['board']['maxrowsperpage'] > 0 && $durl % $cfg['board']['maxrowsperpage'] > 0)
	|| ($d > 0 && $d >= $totallines))
{
	cot_redirect(cot_url('board', $list_url_path + array('dc' => $dcurl)));
}

$pagenav = cot_pagenav('board', $list_url_path + array('dc' => $dcurl), $d, $totallines, $cfg['board']['maxrowsperpage']);

if(empty($c)){
	$cfg_seotitle = trim($cfg['board']['seo_title']);
...
$out['keywords'] = (!empty($cfg['board']['cat_' . $c]['keywords'])) ? $cfg['board']['cat_' . $c]['keywords'] : $cfg['board']['cat___default']['keywords'];

// Building the canonical URL
$out['canonical_uri'] = cot_url('board', $pageurl_params);

$_SESSION['cat'] = $c;

$mskin = cot_tplfile(array('board', 'list', $cat['tpl']));

/* === Hook === */
foreach (cot_getextplugins('board.list.main') as $pl)
{
	include $pl;
}
/* ===== */
...
$t->assign(array(
	...
	'LIST_CAT' => $c,
	'LIST_CAT_RSS' => cot_url('rss', "m=board&c=$c"),
	'LIST_CATTITLE' => $cat['title'],
	'LIST_CATPATH' => $catpath,
	'LIST_CATSHORTPATH' => $catpath_short,
	'LIST_CATURL' => cot_url('board', $list_url_path),
...
{
	$t->assign(array(
		'LIST_SUBMITNEWFIRM' => cot_rc('adv_submitnewboard', array('sub_url' => cot_url('board', 'm=add&c='.$c))),
		'LIST_SUBMITNEWADV_URL' => cot_url('board', 'm=add&c='.$c)
	));
}
...
$pagenav_cat = cot_pagenav('board', $list_url_path + array('d' => $durl), $dc, count($allsub), $cfg['board']['maxlistsperpage'], 'dc');
...
$jj = 0;
/* === Hook - Part1 : Set === */
$extp = cot_getextplugins('board.list.loop');
/* ===== */
$sqllist_rowset = $sqllist->fetchAll();

$sqllist_rowset_other = false;
/* === Hook === */
foreach (cot_getextplugins('board.list.before_loop') as $pl)
{
	include $pl;
}
/* ===== */
...
if(!$sqllist_rowset_other)
{
	...
	{
		$jj++;
		$t->assign(cot_generate_advtags($adv, 'LIST_ROW_', $cfg['board']['truncateadvtext'], $usr['isadmin']));
		$t->assign(array(
			'LIST_ROW_OWNER' => cot_build_user($adv['adv_ownerid'], htmlspecialchars($adv['user_name'])),
			'LIST_ROW_ODDEVEN' => cot_build_oddeven($jj),
			'LIST_ROW_NUM' => $jj
		));
		$t->assign(cot_generate_usertags($adv, 'LIST_ROW_OWNER_'));
...
/* === Hook === */
foreach (cot_getextplugins('board.list.tags') as $pl)
{
	include $pl;
}
/* ===== */
