<!-- BEGIN: MAIN -->
	<script type="text/javascript">
		...
		<h2>{PHP.L.Afisha} ({ADMIN_ADV_TOTALDBFIRMS})</h2>
		...
		<div class=" button-toolbar block">
				...
				<a href="{ADMIN_ADV_URL_ADD}" class="button special">{PHP.L.event_addtitle}</a>
		</div>
		<div class="block">
			<h3>{PHP.L.Afisha}:</h3>
...
<!-- BEGIN: ADV_ROW -->
				<tr>
					...
					<td class="action {ADMIN_ADV_ODDEVEN}">
						<!-- IF {PHP.row.event_state} == 1 --><a title="{PHP.L.Validate}" href="{ADMIN_ADV_URL_FOR_VALIDATED}" class="conafishaLink button">{PHP.L.Validate}</a><!-- ENDIF -->
						<a title="{PHP.L.Delete}" href="{ADMIN_ADV_URL_FOR_DELETED}" class="conafishaLink button">{PHP.L.short_delete}</a>
...
<!-- ENDIF -->
				<tr>
					<td class="valid" colspan="4">
						<!-- IF {PHP.filter} != {PHP.L.adm_validated} --><input name="paction" type="submit" value="{PHP.L.Validate}" onclick="this.form.paction.value=this.value" class="conafisha" /><!-- ENDIF -->
						<input name="paction" type="submit" value="{PHP.L.Delete}" onclick="this.form.paction.value=this.value" />
					</td>
				</tr>
			</table>
			<p class="paging">
				{ADMIN_ADV_PAGINATION_PREV}{ADMIN_ADV_PAGNAV}{ADMIN_ADV_PAGINATION_NEXT}<span>{PHP.L.Total}: {ADMIN_ADV_TOTALITEMS}, {PHP.L.Onafisha}: {ADMIN_ADV_ON_FIRM}</span>
			</p>
			</form>
		</div>
<!-- END: MAIN -->