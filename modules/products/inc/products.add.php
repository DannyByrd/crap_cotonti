...
$prdadd_array = array(
	...
	'PRDADD_FORM_DESC' => cot_textarea('rprddesc', $rprd['prd_desc'], 2, 64, '', 'input_textarea_editor'),
...
foreach($cot_extrafields[$db_products] as $exfld)
{
	...
	if($exfld ["field_type"] == 'filter'){
		$t->assign(array(
			'PRDEDIT_FORM_FILTER_LABEL' => $exfld_title,
			'PRDEDIT_FORM_FILTER_FLD' => $exfld_val,
		));
...
if(cot_plugin_active('sample'))
{
	require_once cot_incfile('sample', 'plug');
	$t->assign('PRDADD_FORM_SAMPLE', sample_generate_select_box());
}
