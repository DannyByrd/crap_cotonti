...
function cot_build_structure_products_tree($c = '',$sub_cat = true, $allsublev = true, $custom_tpl = '', $col = 'all')
{
...
function cot_generate_prdtags($prd_data, $tag_prefix = '', $textlength = 0, $admin_rights = null, $productspath_home = false, $emptytitle = '')
{
	...
	if ($prd_data['prd_id'] > 0 && !empty($prd_data['prd_title']))
	{
		...
		$productspath = cot_products_buildpath($prd_data['prd_cat'], $cfg['products']['bc_prds_link']);
...
function cot_products_import($source = 'POST', $rprd = array(), $auth = array())
{
	...
	$rprd['prd_desc']     = cot_import('rprddesc', $source, 'HTM');
...
function cot_products_add(&$rprd, $auth = array())
{
	...
	{
		$prd_count = $db->query("SELECT COUNT(*) FROM $db_products WHERE prd_alias = ? AND prd_cat = ?", array($rprd['prd_alias'], $rprd['prd_cat']))->fetchColumn();
		if ($prd_count > 0)
		{
...
function cot_products_update($id, &$rprd, $auth = array())
{
	...
	if (!empty($rprd['prd_alias']))
	{
		$prd_count = $db->query("SELECT COUNT(*) FROM $db_products WHERE prd_alias = ? AND prd_id != ? AND prd_cat = ?", array($rprd['prd_alias'], $id, $rprd['prd_cat']))->fetchColumn();
		if ($prd_count > 0)
		{
			$rprd['prd_alias'] = $rprd['prd_alias'].rand(1000, 9999);
		}
