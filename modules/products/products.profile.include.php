...
$profilemenu['products']['title'] = $L['Products'];
$profilemenu['products']['url'] = cot_url('profile', 'm=products');

if($m == 'all')
{
	$user_id = $usr['isadmin'] ? '1' : $usr['id'];
	
	$sql_prd_string = "SELECT f.*, u.*
		FROM $db_products as f
		LEFT JOIN $db_users AS u ON u.user_id=f.prd_ownerid
		WHERE f.prd_ownerid = $user_id AND f.prd_state = 0
		ORDER BY f.prd_date DESC LIMIT 0, 2";

	$prd_set = $db->query($sql_prd_string)->fetchAll();

	$t1 = new XTemplate(cot_tplfile(array('products', 'profile', 'all'), 'module'));

	foreach ($prd_set as $prd)
	{
		$jj++;
		$t1->assign(cot_generate_prdtags($prd, 'LIST_ROW_', $cfg['products']['truncatetext'], $usr['isadmin']));
		$t1->assign(array(
...
elseif($m == 'products')
{
	$t1 = new XTemplate(cot_tplfile(array('products', 'profile'), 'module'));
	...
	/* === Hook === */
	foreach (cot_getextplugins('products.profile.first') as $pl)
	{
		include $pl;
	}
	/* ===== */
	...
	$cfg['products']['truncatefirmtext'] = (empty($c) || $c == 'unvalidated' || $c == 'saved_drafts') ?
		$cfg['products']['cat___default']['truncatefirmtext'] :
		$cfg['products']['cat_' . $c]['truncatefirmtext'];

	$where = array();
	$params = array();

	$where['ownerid'] = "prd_ownerid=".$urr['user_id'];
	...
	$list_url_path = array('m' => 'products', 'c' => $c, 'ord' => $o, 'p' => $p);
	...
	$productsurl_params = array('c' => $c, 'ord' => $o, 'p' => $p);
	if ($durl > 1)
	{
		$productsurl_params['d'] = $durl;
	}
	if ($dcurl > 1)
	{
		$productsurl_params['dc'] = $dcurl;
	}

	/* === Hook === */
	foreach (cot_getextplugins('products.profile.query') as $pl)
	{
		include $pl;
	}
	/* ===== */
	...
	if(empty($sql_prd_string))
	{
		...
		$sql_prd_count = "SELECT COUNT(*) FROM $db_products as f $join_condition $where";
		$sql_prd_string = "SELECT f.*, u.* $join_columns
			FROM $db_products as f $join_condition
			LEFT JOIN $db_users AS u ON u.user_id=f.prd_ownerid
			$where
	...
	$pagenav = cot_pagenav('profile', $list_url_path + array('dc' => $dcurl), $d, $totallines, $cfg['products']['maxrowsperpage']);
	...
	{
		$t1->assign(array(
			'LIST_SUBMITNEWPRD' => cot_rc('prd_submitnewprd', array('sub_url' => cot_url('products', 'm=add'))),
			'LIST_SUBMITNEWPRD_URL' => cot_url('products', 'm=add')
		));
	}
	...
	$pagenav_cat = cot_pagenav('profile', $list_url_path + array('d' => $durl), $dc, count($allsub), $cfg['products']['maxlistsperpage'], 'dc');
	...
	$jj = 0;
	/* === Hook - Part1 : Set === */
	$extp = cot_getextplugins('products.profile.loop');
	/* ===== */
	$sqllist_rowset = $sqllist->fetchAll();

	$sqllist_rowset_other = false;
	/* === Hook === */
	foreach (cot_getextplugins('products.profile.before_loop') as $pl)
	{
		include $pl;
	}
	/* ===== */
	...
	/* === Hook === */
	foreach (cot_getextplugins('products.profile.tags') as $pl)
	{
		include $pl;
	}
	/* ===== */
