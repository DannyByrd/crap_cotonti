...
$profilemenu['firms']['title'] = $L['Firms'];
$profilemenu['firms']['url'] = cot_url('profile', 'm=firms');

if($m == 'all')
{
	$user_id = $usr['isadmin'] ? '1' : $usr['id'];
	
	$sql_firm_string = "SELECT f.*, u.*
		FROM $db_firms as f
		LEFT JOIN $db_users AS u ON u.user_id=f.firm_ownerid
		WHERE f.firm_ownerid = $user_id AND f.firm_state = 0
		ORDER BY f.firm_date DESC LIMIT 0, 2";

	$firm_set = $db->query($sql_firm_string)->fetchAll();

	$t1 = new XTemplate(cot_tplfile(array('firms', 'profile', 'all'), 'module'));

	foreach ($firm_set as $firm)
	{
		$jj++;
		$t1->assign(cot_generate_firmtags($firm, 'LIST_ROW_', $cfg['firms']['truncatetext'], $usr['isadmin']));
		$t1->assign(array(
			'LIST_ROW_OWNER' => cot_build_user($firm['firm_ownerid'], htmlspecialchars($firm['user_name'])),
			'LIST_ROW_ODDEVEN' => cot_build_oddeven($jj),
			'LIST_ROW_NUM' => $jj
		));
		$t1->assign(cot_generate_usertags($firm, 'LIST_ROW_OWNER_'));
...
elseif($m == 'firms')
{
	$t1 = new XTemplate(cot_tplfile(array('firms', 'profile'), 'module'));
	...
	list($pg, $d, $durl) = cot_import_pagenav('d', $maxrowsperpage); //firms number for firms list
	...
	/* === Hook === */
	foreach (cot_getextplugins('firms.profile.first') as $pl)
	{
		include $pl;
	}
	/* ===== */
	...
	$cfg['firms']['maxrowsperpage'] = (empty($c) || $c == 'unvalidated' || $c == 'saved_drafts') ?
		$cfg['firms']['cat___default']['maxrowsperpage'] :
		$cfg['firms']['cat_' . $c]['maxrowsperpage'];
	$cfg['firms']['maxrowsperpage'] = $cfg['firms']['maxrowsperpage'] > 0 ? $cfg['firms']['maxrowsperpage'] : 1;
	...
	$where['ownerid'] = "firm_ownerid=".$urr['user_id'];
	...
	$list_url_path = array('m' => 'firms', 'c' => $c, 'ord' => $o, 'p' => $p);
	...
	/* === Hook === */
	foreach (cot_getextplugins('firms.profile.query') as $pl)
	{
		include $pl;
	}
	/* ===== */
	...
	if(empty($sql_firm_string))
	{
		...
		$sql_firm_string = "SELECT f.*, u.* $join_columns
			...
			LEFT JOIN $db_users AS u ON u.user_id=f.firm_ownerid
			$where
			ORDER BY $orderby LIMIT $d, ".$cfg['firms']['maxrowsperpage'];
	}
	$totallines = $db->query($sql_firm_count, $params)->fetchColumn();
	$sqllist = $db->query($sql_firm_string, $params);

	if ((!$cfg['easypagenav'] && $durl > 0 && $cfg['firms']['maxrowsperpage'] > 0 && $durl % $cfg['firms']['maxrowsperpage'] > 0)
		|| ($d > 0 && $d >= $totallines))
	{
		cot_redirect(cot_url('profile', $list_url_path + array('dc' => $dcurl)));
	}

	$pagenav = cot_pagenav('profile', $list_url_path + array('dc' => $dcurl), $d, $totallines, $cfg['firms']['maxrowsperpage']);
	...
	{
		$t1->assign(array(
			'LIST_SUBMITNEWFIRM' => cot_rc('firm_submitnewfirms', array('sub_url' => cot_url('firms', 'm=add'))),
			'LIST_SUBMITNEWFIRM_URL' => cot_url('firms', 'm=add')
		));
	}
	...
	$pagenav_cat = cot_pagenav('profile', $list_url_path + array('d' => $durl), $dc, count($allsub), $cfg['firms']['maxlistsperpage'], 'dc');
	...
	$jj = 0;
	/* === Hook - Part1 : Set === */
	$extp = cot_getextplugins('firms.profile.loop');
	/* ===== */
	$sqllist_rowset = $sqllist->fetchAll();

	$sqllist_rowset_other = false;
	/* === Hook === */
	foreach (cot_getextplugins('firms.profile.before_loop') as $pl)
	{
		include $pl;
	}
	/* ===== */
	...
	/* === Hook === */
	foreach (cot_getextplugins('firms.profile.tags') as $pl)
	{
		include $pl;
	}
	/* ===== */
