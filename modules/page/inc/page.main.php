...
if ($id > 0 || !empty($al))
{
	$where = (!empty($al)) ? "p.page_cat='".$c."' AND p.page_alias='".$al."'" : 'p.page_id='.$id.' AND p.page_alias=""';
...
$alt_href_en = 'page_ALTERNATE1';

	if(isset($pag[$alt_href_en])){

		
		$r = array();
		preg_match('/\((.+)\)/', $pag[$alt_href_en], $r);
 		$alt_url = str_replace($r[0], '', $pag[$alt_href_en]);
 		$alt_hreflang = $r[1];
		
		$out['alt_hreflang1'] = $alt_hreflang;
		$out['alt_url1'] = $alt_url;
	}

$set_field = explode(',',$pag['page_settariff'] );
...
if($pag['page_tariff'] != 'null' && !$usr['isadmin']){
	...
	if(!$exist_tar){
		...
		foreach ($set_field as $key_field) {
			
			$pag[$key_field] = 'YOU HAVE NO PERMISSION!';
		}
...
$page_tpl = trim($pag['page_tpl']);
if($page_tpl){
    $mskin = cot_tplfile(array('page', $page_tpl));
} else {
