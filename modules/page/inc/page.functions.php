...
cot::$db->registerTable('pages_in_structure');
...
function cot_page_import($source = 'POST', $rpage = array(), $auth = array())
{
	...
	$str_chosen_tar = trim($str_chosen_tar,',');
	...
	$rpage['page_tpl']    	= cot_import('rpagetpl', $source, 'TXT');
	$rpage['page_title']    = cot_import('rpagetitle', $source, 'TXT');
	$rpage['page_desc']     = cot_import('rpagedesc', $source, 'HTM');
	...
	$rpage['page_settariff']   =  $str_chosen_tar;//cot_import('rpagesettariff', $source, 'TXT');
...
function cot_build_structure_page_tree($c = '', $sub_cat = true, $allsublev = true, $custom_tpl = '', $col = 'all')
{
	global $cot_extrafields, $db_structure, $structure, $cfg, $db, $sys, $db_x;
	$t1 = new XTemplate(cot_tplfile(array('page', 'tree', $custom_tpl), 'module'));
	...
	$allsub = (empty($c)) ? cot_structure_children('page', '', $allsublev, false, true, false,$new_col) : cot_structure_children('page', $c, $allsublev, false, true, false,$new_col);
	$subcat = array_slice($allsub, $dc, $cfg['page']['maxlistsperpage']);
	
	/* === Hook - Part1 : Set === */
	$extp = cot_getextplugins('page.tree.rowcat.loop');
	/* ===== */
	foreach ($subcat as $x)
	{	
		$mtch = $structure['page'][$x]['path'].'.';
	...
	if(is_array($cats))
	{
		...
		foreach($cats as $cat)
		{
			...
			$cat_childs = cot_structure_children('page', $cat);
			$sub_count = 0;
			...
			$t1->assign(array(
				...
				'LIST_ROWCAT_TITLE' => $structure['page'][$cat]['title'],
				'LIST_ROWCAT_DESC' => $structure['page'][$cat]['desc'],
				'LIST_ROWCAT_ICON' => $structure['page'][$cat]['icon'],
			...
			foreach ($cot_extrafields[$db_structure] as $exfld)
			{
				...
				$t1->assign(array(
					...
					'LIST_ROWCAT_'.$uname => cot_build_extrafields_data('structure', $exfld, $structure['page'][$cat][$exfld['field_name']]),
...
{
	global $db, $db_pages, $db_pages_in_structure, $db_structure, $cache, $cfg, $cot_extrafields, $structure, $L;
...
function cot_page_update($id, &$rpage, $auth = array())
{
	...
	if (!empty($rpage['page_alias']))
	{
		$page_count = $db->query("SELECT COUNT(*) FROM $db_pages WHERE page_alias = ? AND page_id != ? AND page_cat = ?", array($rpage['page_alias'], $id, $rpage['page_cat']))->fetchColumn();
		if ($page_count > 0)
		{
			$rpage['page_alias'] = $rpage['page_alias'].rand(1000, 9999);
		}
