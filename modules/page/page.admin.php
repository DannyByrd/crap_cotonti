...
$categories = cot_import('filter_cat', 'R', 'ALP');
$sub_categories = cot_import('filter_subcat', 'R', 'ALP');
...
$common_params = 'm=page&sorttype='.$sorttype.'&sortway='.$sortway.'&filter='.$filter;//.'&categories='.$categories;
...
if(!is_null($sub_categories) && $sub_categories != 'any'){

	$sqlwhere .=  " AND page_cat = '".$db->prep($sub_categories)."'";
}else if(!is_null($categories) && $categories != 'any'){

	$sqlwhere .=  " AND page_cat = '".$db->prep($categories)."'";
}
...
elseif ($a == 'delete')
{
	...
	if ($row = $sql_page->fetch())
	{
		...
		// delete page_id form other categories
		$catsToChangeCount = $db->query("SELECT structure_code FROM $db_pages_in_structure WHERE page_id = " . (int)$id . " GROUP BY structure_code")->fetchAll();
		$db->delete($db_pages_in_structure, "page_id = ?", $id);
		foreach ($catsToChangeCount as $row) {
			$newCount = $db->query("SELECT IFNULL(COUNT(*), 0) AS count FROM $db_pages_in_structure WHERE structure_code = " . $db->quote($row['structure_code']))->fetch();
...
$sql_cat = $db->query("SELECT structure_code,structure_title FROM  $db_structure WHERE structure_path  REGEXP '^[0-9]+$' AND structure_area = 'page' AND structure_code != 'system'");


$res_cat = $sql_cat->fetchAll();

$adm_categories = array();
foreach ($res_cat as $cat) {

	foreach ($cat as $c) {
		$adm_categories[$cat['structure_code']] = $cat['structure_title'];
		
	}


}
$adm_categories['any'] = 'Все';
asort($adm_categories);

$adm_sub_categories = array();


if(!is_null($categories) && $categories != 'any'){

	$sql = $db->query("SELECT structure_path AS sp FROM $db_structure WHERE structure_code = '".$db->prep($categories)."' LIMIT 1");
	$res = $sql->fetch();
	$sp = $res['sp'];

	$sql = $db->query("SELECT structure_code,structure_title  FROM $db_structure WHERE structure_path > $sp AND  structure_path < ".++$sp." AND structure_area = 'page' ");
	$res_subc = $sql->fetchAll();
	
	//$sql_cat = $db->query("SELECT structure_code,structure_title FROM  $db_structure WHERE structure_path  REGEXP '^[0-9]+$' AND structure_area = 'page' AND structure_code != 'system'");


	
	foreach ($res_subc as $sub_cat) {

		foreach ($sub_cat as $c) {
			$adm_sub_categories[$sub_cat['structure_code']] = $sub_cat['structure_title'];
...
if(!empty($adm_sub_categories))
	$adm_sub_categories['any'] = 'Все';
asort($adm_sub_categories);
...
$t->assign(array(
	...
	'ADMIN_PAGE_FILTER_CAT'=>cot_selectbox($categories,'filter_cat',array_keys($adm_categories),array_values($adm_categories), false),
	'ADMIN_PAGE_FILTER_SUBCAT'=>cot_selectbox($sub_categories,'filter_subcat',array_keys($adm_sub_categories),array_values($adm_sub_categories), false),
