...
$profilemenu['blogs']['title'] = $L['Blogs'];
$profilemenu['blogs']['url'] = cot_url('profile', 'm=blogs');

if($m == 'all')
{
	$user_id = $usr['isadmin'] ? '1' : $usr['id'];

	$sql_post_string = "SELECT f.*, u.*
		FROM $db_blogs as f
		LEFT JOIN $db_users AS u ON u.user_id=f.post_ownerid
		WHERE f.post_ownerid = $user_id AND f.post_state = 0
		ORDER BY f.post_date DESC LIMIT 0, 2";

	$post_set = $db->query($sql_post_string)->fetchAll();

	$t1 = new XTemplate(cot_tplfile(array('blogs', 'profile', 'all'), 'module'));

	foreach ($post_set as $post)
	{
		$jj++;
		$t1->assign(cot_generate_blogposttags($post, 'LIST_ROW_', $cfg['blogs']['truncatetext'], $usr['isadmin']));
		$t1->assign(array(
		...
		$t1->assign(cot_generate_usertags($post, 'LIST_ROW_OWNER_'));
...
elseif($m == 'blogs')
{
	$t1 = new XTemplate(cot_tplfile(array('blogs', 'profile'), 'module'));
	...
	/* === Hook === */
	foreach (cot_getextplugins('blogs.profile.first') as $pl)
	{
		include $pl;
	}
	/* ===== */
	...
	$cfg['blogs']['truncatefirmtext'] = (empty($c) || $c == 'unvalidated' || $c == 'saved_drafts') ?
		$cfg['blogs']['cat___default']['truncatefirmtext'] :
		$cfg['blogs']['cat_' . $c]['truncatefirmtext'];

	$where = array();
	$params = array();

	$where['ownerid'] = "post_ownerid=".$urr['user_id'];
	...
	$list_url_path = array('m' => 'blogs', 'c' => $c, 'ord' => $o, 'p' => $p);
	...
	$blogsurl_params = array('c' => $c, 'ord' => $o, 'p' => $p);
	if ($durl > 1)
	{
		$blogsurl_params['d'] = $durl;
	}
	if ($dcurl > 1)
	{
		$blogsurl_params['dc'] = $dcurl;
	}

	/* === Hook === */
	foreach (cot_getextplugins('blogs.profile.query') as $pl)
	{
		include $pl;
	}
	/* ===== */
	...
	if(empty($sql_post_string))
	{
		...
		$sql_post_count = "SELECT COUNT(*) FROM $db_blogs as f $join_condition $where";
		$sql_post_string = "SELECT f.*, u.* $join_columns
			FROM $db_blogs as f $join_condition
			LEFT JOIN $db_users AS u ON u.user_id=f.post_ownerid
			$where
	...
	$pagenav = cot_pagenav('profile', $list_url_path + array('dc' => $dcurl), $d, $totallines, $cfg['blogs']['maxrowsperpage']);
	...
	{
		$t1->assign(array(
			'LIST_SUBMITNEWPOST' => cot_rc('post_submitnewpost', array('sub_url' => cot_url('blogs', 'm=add'))),
			'LIST_SUBMITNEWPOST_URL' => cot_url('blogs', 'm=add')
		));
	}
	...
	$pagenav_cat = cot_pagenav('profile', $list_url_path + array('d' => $durl), $dc, count($allsub), $cfg['blogs']['maxlistsperpage'], 'dc');
	...
	$jj = 0;
	/* === Hook - Part1 : Set === */
	$extp = cot_getextplugins('blogs.profile.loop');
	/* ===== */
	$sqllist_rowset = $sqllist->fetchAll();

	$sqllist_rowset_other = false;
	/* === Hook === */
	foreach (cot_getextplugins('blogs.profile.before_loop') as $pl)
	{
		include $pl;
	}
	/* ===== */
	...
	/* === Hook === */
	foreach (cot_getextplugins('blogs.profile.tags') as $pl)
	{
		include $pl;
	}
	/* ===== */
